...
for neuron_index, neuron in enumerate(validNeurons):
    ...
    ax1.plot(T[T_D_index:max_valid_index], np.zeros(max_valid_index-T_D_index)+R_tot, color = green, linewidth=1.5, linestyle='--')
    # R_tot_CI_lo = R_tot - 2 *  R_tot_std
    # R_tot_CI_hi = R_tot + 2 *  R_tot_std
    # ax1.fill_between(T[T_D_index:max_valid_index], np.zeros(max_valid_index-T_D_index)+R_tot_CI_lo, np.zeros(max_valid_index-T_D_index)+R_tot_CI_hi, facecolor=green, alpha=0.3)
