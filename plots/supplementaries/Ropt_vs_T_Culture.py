...
# shuffled
ax1.plot([T_D_new], [ymin], marker='d', markersize = 5., color=green,
         zorder=8)
ax1.plot([T_D_new], [R_tot_new], marker='x', markersize = 6., color=green,
         zorder=8)
ax1.axvline(x=T_D_new, ymax=(R_tot_new - ymin) / yrange, color=green,
            linewidth=0.5, linestyle='--')
x = (np.log10(T_D_new) - np.log10(x_min)) / \
    (np.log10(x_max) - np.log10(x_min))
ax1.axhline(y=R_tot_new, xmax=x, color=green,
            linewidth=0.5, linestyle='--')
ax1.plot([x_min], [R_tot_new], marker='d',markersize = 5., color=green,
         zorder=8)

ax1.plot(T[T_D_index:max_valid_index], np.zeros(max_valid_index-T_D_index)+R_tot_new, color = '0.2',linestyle='--')
