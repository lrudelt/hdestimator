...
for neuron_index in range(number_valid_neurons):
    ...
    R_tot_EC += [R_tot-R[T_offset]]
    # R_tot_EC += [R_tot]

    T_D_EC += [T_D*1000]
    # T_D_EC += [T[R>R_tot*portion][0]*1000]
...
R_tot_EC_mean = np.mean(R_tot_EC)
R_tot_EC_mean_loCI, R_tot_EC_mean_hiCI = plots.get_CI_mean(R_tot_EC)
...
for neuron_index in range(number_valid_neurons):
    ANALYSIS_DIR, analysis_num_str, R_tot, T_D, T, R, R_CI_lo, R_CI_hi = plots.load_analysis_results(
        recorded_system, rec_length, neuron_index, setup, ESTIMATOR_DIR, regularization_method='shuffling')
    R_tot, T_D_index, max_valid_index = plots.get_R_tot(T, R, R_CI_lo)
    R_tot_Retina += [R_tot-R[T_offset]]
    # R_tot_Retina += [R_tot]
    T_D_Retina += [T_D*1000]
    # T_D_Retina += [T[R>R_tot*portion][0]*1000]
    # T_D_new, R_tot_new, R_tot_std_new, T_D_index, max_valid_index = plots.get_temporal_depth_and_R_tot(T, R)
...
R_tot_Retina_mean = np.mean(R_tot_Retina)
R_tot_Retina_mean_loCI, R_tot_Retina_mean_hiCI = plots.get_CI_mean(R_tot_Retina)
...
for neuron_index in range(number_valid_neurons):
    ANALYSIS_DIR, analysis_num_str, R_tot, T_D, T, R, R_CI_lo, R_CI_hi = plots.load_analysis_results(
        recorded_system, rec_length, neuron_index, setup, ESTIMATOR_DIR, regularization_method='shuffling')
    R_tot, T_D_index, max_valid_index = plots.get_R_tot(T, R, R_CI_lo)
    R_tot_Culture += [R_tot-R[T_offset]]
    # R_tot_Culture += [R_tot]
    T_D_Culture += [T_D*1000]
    # T_D_Culture += [T[R>R_tot*portion][0]*1000]
    # T_D_new, R_tot_new, R_tot_std_new, T_D_index, max_valid_index = plots.get_temporal_depth_and_R_tot(T, R)
...
R_tot_Culture_mean = np.mean(R_tot_Culture)
R_tot_Culture_mean_loCI, R_tot_Culture_mean_hiCI = plots.get_CI_mean(R_tot_Culture)
...
for neuron_index in range(number_valid_neurons):
    ANALYSIS_DIR, analysis_num_str, R_tot, T_D, T, R, R_CI_lo, R_CI_hi = plots.load_analysis_results(
        recorded_system, rec_length, neuron_index, setup, ESTIMATOR_DIR, regularization_method='shuffling')
    R_tot, T_D_index, max_valid_index = plots.get_R_tot(T, R, R_CI_lo)
    R_tot_V1 += [R_tot-R[T_offset]]
    # R_tot_V1 += [R_tot]
    # T[R>R_tot*0.8][0]
    # T_D_V1 += [T[R>R_tot*portion][0]*1000]
    T_D_V1 += [T_D*1000]
    # T_D_new, R_tot_new, R_tot_std_new, T_D_index, max_valid_index = plots.get_temporal_depth_and_R_tot(T, R)
...
R_tot_V1_mean = np.mean(R_tot_V1)
R_tot_V1_mean_loCI, R_tot_V1_mean_hiCI = plots.get_CI_mean(R_tot_V1)
...
print(T[T_offset]*1000)
ax.set_ylabel(
    r'\begin{center}remaining history dependence \\$\Delta \hat{R}(80\,\mathrm{ms})$\end{center}')
ax.set_ylim((-0.005, 0.05))
ax.set_yticks([0.0, 0.02, 0.04])
ax.spines['left'].set_bounds(-.005, .05)
...
ax.errorbar(x=[T_D_EC_median], y=[R_tot_EC_mean],  yerr=[[R_tot_EC_mean-R_tot_EC_mean_loCI], [R_tot_EC_mean_hiCI-R_tot_EC_mean]], xerr=[[T_D_EC_median-T_D_EC_median_loCI], [T_D_EC_median_hiCI-T_D_EC_median]], color=main_blue, marker='D', markersize=6)
...
#
# Either plot with, or without EC
# plt.savefig('{}/DR_vs_Tdepth_scatter.pdf'.format(PLOTTING_DIR),
#             format="pdf", bbox_inches='tight')
# plt.savefig('{}/DR_vs_Tdepth_scatter_withoutEC.pdf'.format(PLOTTING_DIR),
#             format="pdf", bbox_inches='tight')
