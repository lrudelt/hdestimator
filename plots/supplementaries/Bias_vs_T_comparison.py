...
for j, setup in enumerate(setups):
    for k, regularization in enumerate(['bbc', 'shuffling']):
        ...
        if setup == 'full_noCV':
            # ax.set_xlabel(r'past range $T$ [sec]')
            ax.set_xlabel(r'recording time $T_{rec}$ (min)')
            ax.set_ylabel(r'relative bias in $R(T)$')

        if setup == 'full_noCV':
            ax.set_title('{}, no cross-validation'.format(regularization))
        else:
            ax.set_title('{}, with cross-validation'.format(regularization))
        ...
        for i, rec_length in enumerate(rec_lengths):
            ...
            median_CI_rel_bias_T_D_val = median_CI_rel_bias_T_D['{}-{}-{}'.format(
                regularization, setup, rec_length)]
            err_lo = median_CI_rel_bias_T_D_val[0] - median_rel_bias_T_D_val
            err_hi = median_CI_rel_bias_T_D_val[1] - median_rel_bias_T_D_val
            ...
            #         label=rec_length,
            #         zorder=4)
            ax.errorbar(x=[rec_length_values[i]], y=[median_rel_bias_T_D_val], yerr=[[-err_lo], [err_hi]],
...
# plt.savefig('{}/Ropt_vs_T_comparison_all_examples.pdf'.format(PLOTTING_DIR),
