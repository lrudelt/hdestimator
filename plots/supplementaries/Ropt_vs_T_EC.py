...
for neuron_index in np.arange(28):
    # neuron_index =14
    print(neuron_index)
    """Load data full"""
    ...
    """Load data full"""
    setup = 'full_bbc'
    ANALYSIS_DIR, analysis_num_str, R_tot_bbc, T_D_bbc, T, R_bbc, R_bbc_CI_lo, R_bbc_CI_hi = plots.load_analysis_results(recorded_system, rec_length, neuron_index, setup, ESTIMATOR_DIR, regularization_method = 'bbc')
    ...
    max_index = np.argmax(R_fivebins)
    ...
    ax.plot([x_min], [R_tot_bbc], marker='d',markersize = 5., color=main_red,
             zorder=8)
    ax.text(0.007, R_tot_bbc + 0.06 *
             R_tot_bbc, r'$\hat{R}_{\mathrm{tot}}$')
    ax.text(T_D_bbc + 0.2 * T_D_bbc, .005, r'$\hat{T}_D$')
    ...
    ax.fill_between(T, R_shuffling_CI_lo, R_shuffling_CI_hi,
                facecolor=main_blue, zorder= 10, alpha=0.3)
